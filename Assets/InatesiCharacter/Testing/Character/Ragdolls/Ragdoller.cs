...
namespace InatesiCharacter.Testing.Character.Ragdolls
{
    public sealed class Ragdoller
    {
        ...
        static void AddComponentesToBase(RagdollPartBase part, RagdollProperties ragdollProperties, float mass, bool addJoint)
        {
            ...
            if (ragdollProperties.onlyCollider == false)
            {
        ...
        public static void CopyBoneTransforms(Transform hipsBoneTransforms, Transform copyFrom)
        {
            if (hipsBoneTransforms == null || copyFrom == null)
                return;

            var bones = hipsBoneTransforms.GetComponentsInChildren<Transform>();
            var bones2 = copyFrom.GetComponentsInChildren<Transform>();

            if (bones == null || bones2 == null)
                return;

            for (int boneIndex = 0; boneIndex < bones2.Length; boneIndex++)
            {
                if (bones[boneIndex] == null) continue;
                bones[boneIndex].localPosition = bones2[boneIndex].localPosition;
                bones[boneIndex].localRotation = bones2[boneIndex].localRotation;
            }
        }
    }
}
