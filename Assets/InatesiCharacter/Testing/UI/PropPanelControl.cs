...
namespace InatesiCharacter.Testing.Stuff
{
    public class PropPanelControl : MonoBehaviour
    {
        ...
        private void BotToggleHandleCallback(ChangeEvent<bool> evt)
        {
            ...
            foreach(var entity in _setupLeoEcs.EcsWorld.Filter<BotComponent>().End())
            {
                ref var botComponent = ref _setupLeoEcs.EcsWorld.GetPool<BotComponent>().Get(entity);
        ...
        private void UpdateContainer()
        {
            ...
            {
                int i = 1;
                var curlan = GameToolkit.Localization.Localization.Instance.CurrentLanguage;
                string nameProp = "";
                ...
                foreach (var prop in _Props)
                {
                    ...
                    item.Q<Button>().text = string.Format(nameProp, i);
...
;
        ...
        {
            var spawnEntity = _setupLeoEcs.EcsWorld.NewEntity();
            var spawnPool = _setupLeoEcs.EcsWorld.GetPool<SpawnComponentEvent>();
        ...
        {
            var playerFilter = _setupLeoEcs.EcsWorld.Filter<CharacterComponent>().Inc<PlayerComponent>().End();
            var playerPool = _setupLeoEcs.EcsWorld.GetPool<CharacterComponent>();
            var charactersPool = _setupLeoEcs.EcsWorld.GetPool<CharacterComponent>();
